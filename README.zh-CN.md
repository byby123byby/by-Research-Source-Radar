...
你有没有遇到过这种情况：让 AI 帮你找和 essay、论文或研究项目相关的资料，结果给出的内容看起来很多，但真正贴合你的问题的很少，有时甚至不如你自己找到的几篇论文或几个开源项目？

Research Source Radar 就是为了解决这个问题。你可以给它一个研究问题、项目名称、论文、截图、视频线索或模糊描述。它会找到原始来源，扩展相关论文和开源项目，并说明哪些内容可以直接使用，哪些只能作为研究思路借鉴。

它还会检查来源是否真的存在，保留没有解决的线索，并指出来源、实现、证据和最终结论之间可能缺少的环节。
...
它不只是返回一串链接，而是把候选资料分成：可以直接使用的内容、需要适配后才能迁移的机制、只能作为背景参考的资料，以及来源身份或证据仍不确定的候选。它还会保留检索范围、验证结果、未解决的缺口和判断依据，方便之后复查。
...
如果你发现遗漏的项目、错误的来源身份、相关性不高的结果、不完整的机制迁移，或者可以复现的问题，欢迎[提交 issue](https://github.com/byby123byby/by-Research-Source-Radar/issues/new)。一份有帮助的反馈最好包含研究问题或线索、你原本期待的结果、实际观察到的结果、相关公开链接，以及使用日期或工作模式。请先删去私人文稿、参与者数据、API 密钥和其他敏感信息。

也欢迎通过 Pull Request 贡献修改。新增检索或验证行为时，请保留来源追溯和主张边界，补充必要文档，并在条件允许时加入针对性测试。小而可复现的修改比夹杂许多无关内容的大改动更容易审查。
