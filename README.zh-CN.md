...
**广泛发现，验证身份，迁移机制，审计每一条主张。**

![状态：发布前预览](https://img.shields.io/badge/status-pre--release-F59E0B)
...
> **发布前预览。** 这个仓库尚未公开，最终安装地址也尚未确定。

这是一个可移植的 Agent Skill，用于可复现地发现研究资料、通过权威元数据验证来源身份、把来源机制迁移到目标场景，并审计“来源、实现、证据和最终主张”是否完整对应。它会把两个问题分开判断：**这个来源能否直接使用？** 以及 **它的机制或证据能否迁移？**

## 快速开始

在原生支持 Agent Skills 的宿主中，可以明确调用 `$research-discovery-and-translation-audit`，也可以直接描述任务；在指令文件型宿主中，适配入口会把匹配任务路由到同一份 `SKILL.md`。
